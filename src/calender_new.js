...
function Calender() {
    ...
    this.selectDay = new Date();  // 当前选中的日期，注意，选中的日期和展示的日期不同
    this.currentShowDate = new Date(); //当前展示的日期
    this.prevMoth = new Date().setMonth(this.currentShowDate.getMonth() - 1); // 当前展示日期的前一个月
    this.nextMonth = new Date().setMonth(this.currentShowDate.getMonth() + 1); // 当前展示日期的下一个月
...
function createCalender(cln) {
    ...
    var dateInfo =getDateInfo(new Date(cln.currentShowDate)),
        date = cln.currentShowDate,
        today =cln.today,
        dates = cln.DOM.dates,
        headTitle = cln.DOM.headTitle,
        infoTittle = cln.DOM.infoTittle,
        infoDate = cln.DOM.infoDate,
        prevDays = dateInfo.weekday - 1,
        ul = document.createElement("ul"),
        eventDay,
        eventNumber;
    ...
    headTitle.innerText = dateInfo.headTitle;
    infoTittle.innerText = getDateInfo(today).infoTittle;
    infoDate.innerText = getDateInfo(today).strDay;
...
function getDateInfo(date) {
    ...
    return {
        ...
        infoTittle:"" + year + "年" + strMonth + "月" + day + "日 " + strWeek
    }
}
